...
<body>
  <div class="row">
    <div>
      <h1>RECIBO DE PAGO A PROVEEDOR</h1>
      <div class="muted">Documento: <b>{{ $pago->folio }}</b></div>
    </div>
    <div class="right">
      <div><b>Folio partida:</b> {{ $partida->folio_partida }}</div>
  ...
  <div class="box mt">
    <h2>Proveedor</h2>
    <div class="mt">
      <div><b>Nombre:</b> {{ $proveedorNombre }}</div>
      <div><b>Referencia doc:</b> {{ $pago->referencia ?? '—' }}</div>
      <div><b>Concepto:</b> {{ $pago->concepto ?? '—' }}</div>
    </div>
  </div>
  ...
  <table>
    ...
    <tbody>
      <tr>
        ...
        <td class="right"><b>$ {{ number_format($partida->monto,2) }}</b></td>
      </tr>
    </tbody>
  </table>

  <div class="box mt">
    <div><b>Referencia pago:</b> {{ $partida->referencia_pago ?? '—' }}</div>
